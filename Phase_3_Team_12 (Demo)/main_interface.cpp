...
void createAccountFromStruct(vector<Account>& accounts, string fileName, Account account);
void deleteAccount(vector<Account>&, string, int);
...
void displayAccounts(vector<Account> accounts) {
    cout << left << setw(20) << "Full Name" << "\t\t" << "Account Number" << "\t\t" << "Balance" << "\t\t" << "Admin" << "\t\t" << "Master?" << endl;
    cout << "-----------------------------------------------------------------------------------------------" << endl;
    if (accounts.size() == 0) cout << "                       - No Data -                     \n";
    for (Account account : accounts) {
        cout << left << setw(20) << account.name << "\t\t" << account.accountNumber << "\t\t\t" << account.balance << "\t\t" << account.isAdmin << "\t\t" << account.masterAdmin << endl;
    }
    cout << "-----------------------------------------------------------------------------------------------\n" << endl;
    cout << endl;
}
...
void createAccountFromStruct(vector<Account>& accounts, string fileName, Account account) {
    ...
    account = {account.name, accountNumber, account.password, account.balance, "No", "No"};
    accounts.push_back(account);
    writeAccountsToFile(fileName, accounts);
    cout << "Account for " << account.name << " created successfully.\n" << endl;
}
